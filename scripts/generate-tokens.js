...
const INPUT_FILE = path.resolve(__dirname, '../src/design-system/token/token.const.1tier.ts');
const OUTPUT_FILE = path.resolve(__dirname, '../src/design-system/token/tokens.1tier.css');
...
// Add SizeKeywords
const sizeKeywordsMatches = content.match(/export const SizeKeywords = \[([\s\S]*?)\] as const;/);
if (sizeKeywordsMatches) {
    ...
    keywords.forEach(k => {
        ...
        let val = '';
        switch (k) {
            case 'full': val = '100%'; break;
            case 'screen': val = '100vh'; break;
            case 'min': val = 'min-content'; break;
            case 'max': val = 'max-content'; break;
            case 'fit': val = 'fit-content'; break;
            case 'auto': val = 'auto'; break;
            default: val = 'initial'; // fallback
        }
        css += `  --size-${k}: ${val};\n`;
    });
}
...
const radiusObjMatch = content.match(/export const Radius = \{([\s\S]*?)\} as const/);
if (radiusObjMatch) {
    css += `\n  /* Radius Aliases */\n`;
    const lines = radiusObjMatch[1].split('\n');
    lines.forEach(line => {
        // match:  alias: "radius.nX"
        const match = line.match(/^\s*["']?([\w]+)["']?\s*:\s*["']radius\.n(\d+)["']/);
        if (match) {
            const [_, alias, val] = match;
            // check if it's an numeric key (n0, n2...) - skip those as they are base tokens
            if (alias.startsWith('n') && !isNaN(parseInt(alias.slice(1)))) return;
