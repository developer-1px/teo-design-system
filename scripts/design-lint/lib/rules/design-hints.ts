  ...
  for (const prop of props) {
    ...
    if (propName === "className") {
      issues.push({
        ...
        rule: "no-classname",
        message:
          "Do not use 'className'. Use 'override' or 'style' for custom styling.",
        severity: "error",
      });
    }
        ...
        if (expression) {
          ...
          if (text.includes("Layout.Col.Left")) {
            issues.push({
              ...
              message:
                "Consider using 'Layout.Col.Stretch' for structural containers to ensure children fill the width.",
              severity: "warn",
            });
          }
        }
