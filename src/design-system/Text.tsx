...
export function Text({ children, variant = 3, as, weight, opacity, size, className = '', style: styleProp = {} }: TextProps) {
    // Map hierarchy to automatic tags if not specified
    const Tag = as || (variant === 1 ? 'h1' : variant === 2 ? 'h2' : variant === 3 ? 'p' : 'span');
    ...
    const baseStyle = {
        ...
        fontWeight: weight ? (weight === 'bold' ? 600 : weight === 'medium' ? 500 : 400) : (variant <= 2 ? 'var(--font-weight-bold)' : 'var(--font-weight-regular)'),
        lineHeight: 1.5,
        margin: 0,
        opacity,
