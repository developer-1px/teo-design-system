...
import type { ShadowTokens, TokenInput } from '../types';
...
export function generateShadow(input: TokenInput): ShadowTokens {
  const { role, prominence = 'Standard', state = {} } = input;
  ...
  if (state.hover && isAction && !state.disabled) {
    ...
    if (prominence === 'Hero' || tier === 'Level3') {
      boxShadow =
        'shadow-[0_20px_40px_-5px_rgba(0,0,0,0.15),0_10px_20px_-5px_rgba(0,0,0,0.1)] hover:-translate-y-1 transition-all duration-300';
