...
export function useResizable({
  ...
  return {
    ...
    handleProps: {
      ...
      className: `absolute z-50 transition-colors hover:bg-primary/50 ${
        isResizing ? 'bg-primary' : 'bg-transparent'
      } ${
        direction === 'right'
          ? 'top-0 right-0 w-1 h-full cursor-col-resize hover:w-1.5'
          : direction === 'left'
            ? 'top-0 left-0 w-1 h-full cursor-col-resize hover:w-1.5'
            : direction === 'bottom'
              ? 'bottom-0 left-0 w-full h-1 cursor-row-resize hover:h-1.5'
