...
Tailwind의 `p-4`는 16px입니다. AI는 이미 `4 = 16px`로 학습되어 있습니다.

제가 base를 수정해서 `p-4 = 12px`로 바꿔도, AI는 관성적으로 `p-4`가 16px인 것처럼 디자인했습니다. 설정 파일을 아무리 바꿔도 소용없었습니다. **문자열이라 에러가 나지 않으니 **, AI는 무엇이 틀렸는지조차 알지 못했습니다.

```javascript
// tailwind.config.js에서 spacing을 바꿔도
// AI는 여전히 p-4 = 16px로 생각하고 디자인합니다
```

### CSS-in-JS: 의외로 잘 못한다

styled-components, Emotion 같은 런타임 CSS-in-JS를 시도해봤지만, 예상외로 AI가 이것들을 잘 다루지 못했습니다. 문법의 자유도가 높아서인지, 일관된 패턴으로 코드를 생성하는 데 어려움을 겪었습니다.

### 컴포넌트 Props 기반: inline-style로의 탈출

Chakra UI 스타일로 컴포넌트에 props를 전달하는 방식도 사용해 보았습니다. 문제는 AI가 제가 만든 컴포넌트의 세계를 완전히 이해하지 못한다는 점이었습니다. 학습되지 않은 커스텀 시스템을 다루는 데에는 한계가 있었습니다.

결국 AI는 **inline-style로 탈출 **했습니다.
...
inline-style은 CSS의 가장 큰 한계입니다. hover, active, media query를 사용할 수 없어, 현대 웹 UI를 만들기에는 치명적인 제약이 있습니다.
