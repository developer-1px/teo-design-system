      ...
      >
            ...
            <div className="py-2">
              {results.map((result, index) => (
                <Button
                  ...
                  variant="ghost"
                  {...getItemProps(index)}
                  className={cn('w-full justify-start gap-3 px-4 py-2.5 h-auto', {
                    'bg-accent/10 hover:bg-accent/10': index === cursorIndex,
                  })}
                >
                  {getIcon(result.type)}
                  <div className="flex-1 min-w-0">
                    <div className="text-sm text-text truncate">{result.title}</div>
                    {result.subtitle && (
                      <div className="text-xs text-subtle truncate">{result.subtitle}</div>
                    )}
                  </div>
                  {index === cursorIndex && (
        ...
        <Section role="Footer" prominence="Subtle" className="px-4 py-2">
          <div className="flex items-center justify-between">
            ...
            <div className="text-xs text-subtle">{results.length} results</div>
          </div>
        </Section>
      </Section>
    </div>
