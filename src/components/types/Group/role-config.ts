/**
 * Group Role Configuration (v4.1)
 *
 * Group role → 모든 특성 자동 결정
 * - htmlTag: 시맨틱 HTML 태그
 * - ariaProps: 접근성 속성
 ...
 *
 * **책임 분리**:
 * - Group.tsx: role-config 조회 → 설정 적용 → 렌더링
...
import type { GroupRole, GroupProps } from '@/components/types/Atom/types';
 ...
 * 모든 Group renderer가 공통으로 받는 props
 */
export interface GroupRendererProps extends Omit<GroupProps, 'role'> {
  role: GroupRole;
...
/**
 * Group Role Configuration (전역 - 모든 layout에서 동일)
 *
 * **조직 구조** (v4.1):
 ...
 * - Navigation: 네비게이션
 */
export const ROLE_CONFIGS: Record<GroupRole, GroupRoleConfig> = {
  // ==================== Layout Containers ====================
  Container: {
    htmlTag: 'div',
    baseStyles: '',
  ...
  SortableList: {
    ...
    baseStyles: '',
    renderer: SortableList as ComponentType<GroupRendererProps>,
  ...
  Toolbar: {
    ...
    baseStyles: 'flex items-center gap-2',
    renderer: Toolbar as ComponentType<GroupRendererProps>,
  ...
  Accordion: {
    ...
    renderer: Accordion as ComponentType<GroupRendererProps>,
 ...
 */
export function getRoleConfig(role: GroupRole): GroupRoleConfig {
 ...
 */
export function hasRenderer(role: GroupRole): boolean {
