/**
 * FieldSpinbutton - 숫자 입력 필드 렌더러
 *
 * Headless useFieldSpinbutton + CVA styles 조합
...
import { useFieldSpinbutton } from '../../headless/useFieldSpinbutton';
import {
  errorStyles,
  fieldWrapperStyles,
  inputStyles,
...
export interface FieldSpinbuttonProps {
  /**
   * Field label
   */
  label: string;
...
export function FieldSpinbutton(props: FieldSpinbuttonProps) {
  const {
    label,
    model,
    type,
  ...
  // Headless logic
  const field = useFieldSpinbutton({
    model,
    value,
    required,
    constraints,
