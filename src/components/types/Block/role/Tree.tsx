...
export function Tree({
    ...
    const getIcon = (node: any, isExpanded: boolean) => {
        const iconName = icons[node.type] || (node.type === 'folder' ? (isExpanded ? 'FolderOpen' : 'Folder') : 'File');
    ...
    return (
        <div
            className={cn('w-full py-1 focus:outline-none focus:ring-1 focus:ring-accent/30 rounded', className)}
            tabIndex={0}
            {...rest}
        >
