...
export function Dialog({ Element, children, className, ...props }: BlockRendererProps) {
  return (
    <Element
      className={cn(
        'relative p-6 bg-surface shadow-2xl rounded-xl border border-border-default max-w-sm mx-auto my-4',
        className
      )}
      {...props}
    >
      ...
      <div className="text-sm text-text-subtle mb-4">
        This is a simulated dialog. In a real app, this would be a modal overlay.
      </div>
      {children}
...
export function AlertDialog({ Element, children, className, ...props }: BlockRendererProps) {
  return (
    <Element
      className={cn(
        'bg-surface-warning/10 border-l-4 border-warning p-4 rounded-r-md max-w-sm mx-auto',
        className
      )}
      {...props}
    >
      <h4 className="flex items-center font-bold text-warning mb-1">⚠️ Confirm Deletion</h4>
      <div className="text-sm mb-3">
        Are you sure you want to delete this item? This action cannot be undone.
      </div>
      <div className="flex gap-2">
        <button className="px-3 py-1 bg-warning text-white text-sm rounded font-medium">
          Delete
        </button>
...
export function Sheet({ Element, children, className, spec, ...props }: BlockRendererProps) {
  ...
  return (
    <Element
      className={cn(
        'h-[300px] w-[250px] bg-surface shadow-xl border border-border-default flex flex-col',
...
export function Popover({ Element, children, className, ...props }: BlockRendererProps) {
  return (
    <Element
      className={cn(
        'bg-surface shadow-lg border border-border-default rounded-md p-3 max-w-xs text-sm',
        className
      )}
      {...props}
    >
...
export function Tooltip({ Element, children, className, ...props }: BlockRendererProps) {
  return (
    <Element
      className={cn(
        'bg-surface-inverse text-surface-on-inverse px-2 py-1 rounded text-xs inline-block shadow-md',
        className
      )}
      {...props}
    >
...
export function Toast({ Element, children, className, ...props }: BlockRendererProps) {
  return (
    <Element
      className={cn(
        'bg-surface shadow-lg border border-border-default rounded-md p-3 flex items-center gap-3 w-80 animate-in slide-in-from-bottom-2',
        className
      )}
      {...props}
    >
