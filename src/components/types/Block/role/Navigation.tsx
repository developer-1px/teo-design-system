...
export function Breadcrumbs({ Element, children, className, spec, ...props }: BlockRendererProps) {
    ...
    return (
        <Element className={cn('flex items-center gap-2 text-sm text-text-subtle', className)} {...props}>
            {children || items.map((item, i) => (
                <React.Fragment key={i}>
                    {i > 0 && <span className="opacity-50">/</span>}
                    <span className={cn(i === items.length - 1 ? 'text-text font-medium' : 'hover:text-text cursor-pointer transition-colors')}>
                        {item}
                    </span>
                </React.Fragment>
            ))}
            ...
            {steps.map((step, i) => (
                <div key={i} className="flex flex-1 items-center">
                    <div className={cn(
                        ...
                        i <= current ? 'bg-primary text-white border-primary' : 'bg-surface text-text-subtle border-border-default'
                    )}>
                        {i + 1}
                    </div>
                    <div className={cn(
    ...
    return (
        <Element className={cn('flex gap-6 border-b border-border-default px-4', className)} {...props}>
            {['Overview', 'Features', 'Pricing', 'Docs'].map(item => (
                <div key={item} className="py-3 text-sm font-medium text-text-subtle hover:text-text cursor-pointer border-b-2 border-transparent hover:border-text-subtle transition-all">
                    {item}
                </div>
            ))}
        </Element>
...
export function Sidebar({ Element, children, className, ...props }: BlockRendererProps) {
    return (
        <Element className={cn('w-64 border-r border-border-default bg-surface h-full flex flex-col', className)} {...props}>
            <div className="p-4 font-bold border-b border-border-default">Sidebar</div>
            <div className="flex-1 p-2 space-y-1">
                <div className="px-3 py-2 bg-surface-selected rounded-md text-sm font-medium text-primary">Dashboard</div>
                <div className="px-3 py-2 hover:bg-surface-hover rounded-md text-sm text-text">Settings</div>
...
export function AppBar({ Element, children, className, ...props }: BlockRendererProps) {
    return (
        <Element className={cn('h-14 border-b border-border-default bg-surface/80 backdrop-blur sticky top-0 z-50 flex items-center px-4 justify-between', className)} {...props}>
            <div className="font-bold text-lg">App Title</div>
            <div className="flex items-center gap-4">
                <span className="text-sm">Link 1</span>
                <button className="px-3 py-1.5 bg-primary text-white text-sm rounded-md shadow-sm">Action</button>
            </div>
        </Element>
    );
}
