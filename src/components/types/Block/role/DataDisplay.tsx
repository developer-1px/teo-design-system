        ...
        <Element className={cn('grid grid-cols-[120px_1fr] gap-y-2 text-sm', className)} {...props}>
            {children || (
                <>
                    ...
                    <dd><span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-800">Active</span></dd>
                </>
            )}
        </Element>
    );
...
export function Stats({ Element, children, className, ...props }: BlockRendererProps) {
    return (
        <Element className={cn('bg-surface p-4 rounded-lg border border-border-default', className)} {...props}>
...
export function Avatar({ Element, children, className, spec, ...props }: BlockRendererProps) {
    ...
    return (
        <Element className={cn('w-10 h-10 rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold text-sm border-2 border-surface', className)} {...props}>
...
export function AvatarGroup({ Element, children, className, ...props }: BlockRendererProps) {
    return (
        <Element className={cn('flex -space-x-3', className)} {...props}>
            ...
            <div className="w-10 h-10 rounded-full bg-surface-raised border-2 border-surface flex items-center justify-center text-xs font-medium text-text-subtle">+3</div>
        </Element>
    )
}
...
export function Badge({ Element, children, className, ...props }: BlockRendererProps) {
    return (
        <Element className={cn('inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-primary text-white', className)} {...props}>
            {children || 'New'}
        </Element>
    )
}
...
export function Tag({ Element, children, className, ...props }: BlockRendererProps) {
    return (
        <Element className={cn('inline-flex items-center px-2 py-1 rounded bg-surface-raised border border-border-default text-xs font-medium gap-1', className)} {...props}>
            {children || 'Tag Name'}
...
export function EmptyState({ Element, children, className, ...props }: BlockRendererProps) {
    return (
        <Element className={cn('flex flex-col items-center justify-center py-10 px-4 text-center border-2 border-dashed border-border-default rounded-lg bg-surface-subtle/30', className)} {...props}>
            <div className="w-12 h-12 rounded-full bg-surface-raised flex items-center justify-center text-2xl mb-3">📦</div>
            ...
            <button className="px-3 py-1.5 bg-primary text-white text-sm rounded shadow-sm font-medium">Create New</button>
        </Element>
    )
}
