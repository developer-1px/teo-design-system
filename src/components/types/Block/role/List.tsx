...
export function List({ Element, children, className, ...props }: BlockRendererProps) {
    return (
        <Element className={cn('flex flex-col divide-y divide-border-subtle border border-border-default rounded-md overflow-hidden', className)} {...props}>
            {children || [1, 2, 3].map(i => (
                <div key={i} className="px-4 py-3 hover:bg-surface-hover cursor-pointer transition-colors text-sm">
                    List Item {i}
                </div>
            ))}
        </Element>
...
export function Menu({ Element, children, className, ...props }: BlockRendererProps) {
    return (
        <Element className={cn('min-w-[200px] border border-border-default rounded-md shadow-lg bg-surface py-1', className)} {...props}>
            {children || ['Edit', 'Duplicate', 'Delete'].map(item => (
                <div key={item} className={cn("px-3 py-1.5 text-sm hover:bg-primary hover:text-white cursor-pointer mx-1 rounded-sm", item === 'Delete' && 'text-error hover:bg-error')}>
                    {item}
                </div>
            ))}
        </Element>
...
export function CommandPalette({ Element, children, className, ...props }: BlockRendererProps) {
    return (
        <Element className={cn('max-w-xl mx-auto border border-border-default rounded-xl shadow-2xl bg-surface overflow-hidden flex flex-col', className)} {...props}>
            <div className="flex items-center px-4 border-b border-border-default h-12">
                <span className="text-text-subtle mr-2">🔍</span>
                <input className="flex-1 bg-transparent border-none focus:outline-none text-sm" placeholder="Type a command or search..." />
            </div>
            <div className="max-h-[300px] overflow-y-auto p-2">
                ...
                <div className="px-2 py-1.5 hover:bg-surface-hover rounded-md text-sm text-text-subtle">Run Command</div>
            </div>
        </Element>
    );
}
    ...
    return (
        <Element className={cn('flex flex-col text-sm', className)} {...props}>
            <div className="pl-4 border-l border-border-default ml-2">
                ...
                <div className="pl-4 border-l border-border-default ml-2">
                    <div className="flex items-center gap-1 py-1 hover:text-primary cursor-pointer">📂 components</div>
                    <div className="pl-4 border-l border-border-default ml-2">
                        <div className="flex items-center gap-1 py-1 hover:text-primary cursor-pointer">📄 Block.tsx</div>
                    </div>
                    <div className="flex items-center gap-1 py-1 hover:text-primary cursor-pointer">📄 App.tsx</div>
                </div>
            </div>
        </Element>
    );
