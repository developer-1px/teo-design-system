...
export function Alert({ Element, children, className, ...props }: BlockRendererProps) {
    return (
        <Element className={cn('flex gap-3 p-3 rounded-md bg-info/10 text-info-text border border-info/20', className)} {...props}>
...
export function Progress({ Element, children, className, spec, ...props }: BlockRendererProps) {
    ...
    return (
        <Element className={cn('w-full h-2 bg-surface-raised rounded-full overflow-hidden', className)} {...props}>
...
export function Banner({ Element, children, className, ...props }: BlockRendererProps) {
    return (
        <Element className={cn('w-full bg-primary text-white px-4 py-2 flex items-center justify-between text-sm shadow-md', className)} {...props}>
            <span className="font-medium">✨ New features available!</span>
            <button className="text-white/80 hover:text-white px-2 uppercase text-xs font-bold tracking-wider">Learn More</button>
        </Element>
    )
}
...
export function Callout({ Element, children, className, ...props }: BlockRendererProps) {
    return (
        <Element className={cn('pl-4 border-l-4 border-accent bg-surface-raised/50 py-2 pr-2 my-2 rounded-r', className)} {...props}>
            {children || <div className="text-sm italic text-text-subtle">"This is a highlighted quote or note."</div>}
        </Element>
    )
}
