            ...
            <SidebarHeader
                ...
                actions={
                    <div className="flex items-center gap-1">
                        <Action role="IconButton" icon="Filter" label="Filter Extensions" prominence="Subtle" density="Compact" />
                        <Action role="IconButton" icon="RefreshCw" label="Refresh" prominence="Subtle" density="Compact" />
                        <Action role="IconButton" icon="MoreHorizontal" label="Views and More Actions" prominence="Subtle" density="Compact" />
                    </div>
                }
            />
    ...
    return (
        <Block role="Card" className="flex gap-3 hover:bg-surface-hover p-2 rounded-lg cursor-pointer group bg-transparent border-0 shadow-none">
            ...
            <div className="flex-1 min-w-0 flex flex-col gap-0.5">
                ...
                <Text role="Body" content={description} className="text-xs text-text-secondary line-clamp-2" />
                <div className="flex items-center gap-3 mt-1 text-[10px] text-text-tertiary">
                    <span className="flex items-center gap-0.5"><DownloadCloud size={10} /> {installs}</span>
                    <span className="flex items-center gap-0.5"><Star size={10} className="fill-current" /> {rating}</span>
                    <span>{publisher}</span>
                </div>
            </div>
            ...
            <div className="flex flex-col justify-center">
                {installed ? (
                    <Action role="IconButton" icon="Settings" label="Manage" prominence="Subtle" density="Compact" className="opacity-0 group-hover:opacity-100" />
                ) : (
