        ...
        {searchQuery && (
          <Block role="List" className="flex flex-col gap-0">
            <div className="px-2 py-1 text-xs text-text-tertiary font-medium">
              {mockResults.reduce((acc, curr) => acc + curr.matches.length, 0)} results in{' '}
            ...
            {mockResults.map((result, idx) => (
              <Block key={idx} role="Container" className="flex flex-col">
                ...
                <button className="flex items-center gap-1 px-2 py-1 hover:bg-surface-hover rounded-sm text-left group">
                  ...
                  <span className="text-xs text-text-tertiary bg-surface-raised px-1.5 rounded-full">
                ...
                <div className="flex flex-col ml-6 border-l border-border-muted">
                  {result.matches.map((match, mIdx) => (
                    <button
                      key={mIdx}
                      className="flex items-start gap-2 px-2 py-1 hover:bg-surface-hover text-left overflow-hidden"
                    >
                      <span className="text-xs text-text-tertiary font-mono w-6 text-right flex-shrink-0 mt-0.5">
                        {match.line}
                      </span>
