...
const JsonTree = ({ data, level = 0, name }: any) => {
    ...
    if (!isObject) {
        return (
            <div className="flex items-start gap-1 font-mono text-xs py-0.5 hover:bg-surface-hover rounded-sm px-1 cursor-default" style={{ paddingLeft: `${level * 16}px` }}>
    ...
    return (
        <div>
            ...
            {isOpen && (
                <div className="text-text-tertiary font-mono text-xs py-0.5 px-1 opacity-50" style={{ paddingLeft: `${level * 16 + 20}px` }}>
                    {isArray ? ']' : '}'}
                </div>
            )}
        </div>
