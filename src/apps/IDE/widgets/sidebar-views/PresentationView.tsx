...
import { Play, Square, LayoutTemplate, Image } from 'lucide-react';
            ...
            <SidebarHeader
                ...
                actions={
                    <div className="flex items-center gap-1">
                        <Action role="IconButton" icon="Plus" label="Add Slide" prominence="Subtle" density="Compact" />
                        <Action role="IconButton" icon="Play" label="Start Presentation" prominence="Subtle" density="Compact" />
                    </div>
                }
            />
                    ...
                    {slides.map((slide, idx) => (
                        <div key={idx} className={`flex gap-3 p-2 rounded-lg cursor-pointer transition-colors ${slide.active ? 'bg-surface-raised border border-border-default' : 'hover:bg-surface-hover border border-transparent'}`}>
                            ...
                            <div className="w-20 h-12 bg-surface-sunken rounded border border-border-default flex items-center justify-center relative overflow-hidden">
                                ...
                                <span className="absolute bottom-0.5 right-1 text-[8px] font-mono text-text-tertiary">{idx + 1}</span>
                            </div>

                            {/* Content */}
                            <div className="flex-1 min-w-0 flex flex-col justify-center">
                                <Text role="Body" content={slide.title} className={`text-sm truncate ${slide.active ? 'font-bold text-text' : 'font-medium text-text-secondary'}`} />
