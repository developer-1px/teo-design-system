...
function SmartValueRenderer({ value }: { value: unknown }) {
    ...
    if (typeof value === 'object' && value !== null) {
        return (
            <Text.Field.Value style={{ fontSize: "12px", color: "var(--text-tertiary)" }}>
    ...
    // Primitive
    return (
        <Text.Field.Value style={{ fontSize: "12px", color: "var(--text-secondary)", wordBreak: "break-word" }}>
            {String(value)}
        </Text.Field.Value>
    );
}
...
export function JsonPropertyGrid({ data }: { data: Record<string, any> }) {
    ...
    return (
        <Frame layout={Layout.Grid.Cards.Compact} override={{ gap: Space.n12, columns: "repeat(auto-fill, minmax(140px, 1fr))" }}>
            {entries.map(([key, value]) => (
                <Frame key={key} layout={Layout.Stack.Content.Tight} gap={Space.n2}>
                    <Text.Field.Label style={{ color: "var(--text-tertiary)", fontSize: "11px" }}>
                        {key}
                    </Text.Field.Label>
