...
export function PageShowcasePage() {
  ...
  return (
    <ShowcasePage
      ...
      subtitle="Strict IDDL 5.0 Implementation"
      description="Page components demonstrate the top-level layout zoning and semantic roles."
      sidebar={sidebar}
    >
      <Block role="Container" className="gap-8 w-full max-w-6xl mx-auto h-full">
        {/* Section Toggles */}
        <div className="flex flex-col items-center gap-4 w-full bg-surface p-6 rounded-2xl border border-border-default shadow-lg mb-8 transition-all hover:shadow-xl">
          <div className="flex items-center gap-2 text-xs uppercase tracking-[0.2em] text-text-secondary font-black">
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    ${isDisabled
                      ? 'bg-surface-sunken border-border-subtle text-text-disabled opacity-50'
          ...
          <div className="text-[10px] text-text-secondary font-medium bg-surface-sunken px-3 py-1 rounded-full border border-border-default italic">
        ...
        <Block
          role="Card"
          className="aspect-[4/3] w-full bg-surface-sunken p-8 border border-border-default rounded-xl shadow-inner relative overflow-hidden flex flex-col items-center justify-center flex-1"
        >
          {/* Simulation Window */}
          <Block
            role="DeviceFrame"
            className={`
              border-[6px] border-text-primary overflow-hidden shadow-2xl relative
                  ...
                  <Section key={role} role={role} className="relative group overflow-hidden">
                    <Block
                      ...
                      className={`
                       ...
                       ${isMissing ? 'ring-2 ring-intent-critical ring-inset' : ''} 
                     `}
                    >
                      <div className="p-2 border-b border-black/5 flex items-center justify-center bg-black/5 gap-2">
                        ...
                        {isMissing && <AlertTriangle size={12} className="text-intent-critical" />}
                      </div>
