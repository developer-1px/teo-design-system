...
import {
    ...
    Settings, Share2, Eye, Trash2, Plus, Grid
} from 'lucide-react';
...
interface FormField {
    ...
    colSpan: 1 | 2 | 3 | 4; // Grid specific
}

const INITIAL_FIELDS: FormField[] = [
    { id: 'f1', type: 'text', label: 'First Name', required: true, placeholder: 'e.g. John', colSpan: 2 },
    { id: 'f1b', type: 'text', label: 'Last Name', required: true, placeholder: 'e.g. Doe', colSpan: 2 },
    { id: 'f2', type: 'email', label: 'Email Address', required: true, placeholder: 'john@example.com', colSpan: 4 },
    { id: 'f3', type: 'textarea', label: 'Bio', required: false, description: 'Tell us a bit about yourself', colSpan: 4 },
];
                ...
                <div className={styles.centerPanel}>
                    <div className={styles.canvas} onClick={() => { setSelectedId(null); setEditingId(null); }}>
                        <div className={styles.formContainer} onClick={e => e.stopPropagation()}>
                            {/* Visual Grid Guides */}
                            <div className={styles.gridGuideLayer}>
                            ...
                            <div style={{ gridColumn: '1 / -1', borderBottom: '1px solid #eee', paddingBottom: '16px', marginBottom: '8px', zIndex: 1 }}>
                            ...
                            {fields.map(field => (
                                <CanvasField
                                    ...
                                    onChangeWidth={(cols) => updateField(field.id, { colSpan: cols })}
                                    onDelete={() => removeField(field.id)}
                                />
                            ))}

                            {fields.length === 0 && (
                                <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '40px', color: '#999', border: '2px dashed #eee', borderRadius: '8px', zIndex: 1, position: 'relative' }}>
                            ...
                            <>
                                ...
                                <PropertySection title="Layout">
                                    <div className={styles.propertyRow}>
                                        <div className={styles.propertyLabel}>Width</div>
                                        <div style={{ display: 'flex', gap: 4 }}>
                                            {[1, 2, 3, 4].map(cols => (
                                                <button
                                                    key={cols}
                                                    onClick={() => updateField(selectedField.id, { colSpan: cols as any })}
                                                    style={{
                                                        flex: 1,
                                                        height: 24,
                                                        border: selectedField.colSpan === cols ? '1px solid #111' : '1px solid #ddd',
                                                        background: selectedField.colSpan === cols ? '#111' : 'white',
                                                        color: selectedField.colSpan === cols ? 'white' : '#666',
                                                        borderRadius: 4,
                                                        fontSize: 11,
                                                        cursor: 'pointer'
                                                    }}
...
function CanvasField({ field, isSelected, isEditing, onClick, onEdit, onChangeWidth, onDelete }: {
    field: FormField,
    isSelected: boolean,
    isEditing: boolean,
    onClick: () => void,
    onEdit: (val: string) => void,
    onChangeWidth: (cols: 1 | 2 | 3 | 4) => void,
    onDelete: () => void
}) {
    return (
        <div
            className={styles.canvasField({ colSpan: field.colSpan })}
            data-selected={isSelected}
            onClick={(e) => { e.stopPropagation(); onClick(); }}
        >
            {isSelected && (
                <div className={styles.floatingToolbar} onClick={e => e.stopPropagation()}>
                    <button className={styles.toolbarButton} onClick={() => onChangeWidth(1)} title="1/4 Width">
                        <div style={{ width: 6, height: 10, border: '1px solid currentColor', opacity: field.colSpan === 1 ? 1 : 0.5 }} />
                    </button>
                    <button className={styles.toolbarButton} onClick={() => onChangeWidth(2)} title="Half Width">
                        <div style={{ width: 12, height: 10, border: '1px solid currentColor', opacity: field.colSpan === 2 ? 1 : 0.5 }} />
                    </button>
                    <button className={styles.toolbarButton} onClick={() => onChangeWidth(4)} title="Full Width">
                        <div style={{ width: 24, height: 10, border: '1px solid currentColor', opacity: field.colSpan === 4 ? 1 : 0.5 }} />
                    </button>
                    <div className={styles.toolbarDivider} />
                    <button className={styles.toolbarButton} onClick={onDelete} style={{ color: '#F87171' }} title="Delete">
                        <Trash2 size={14} />
                    </button>
                </div>
            )}
